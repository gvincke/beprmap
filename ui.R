...
#                 )),
    ...
    HTML('<hr style="border:1px solid #ccc;"/>'),
    h5(HTML("Afficher")),
    checkboxInput("zones", label = "Afficher les zones pour le demi-fond et le fond", value = FALSE),
    br(),
  ...
  mainPanel(
	...
	tabPanel(
	  ...
	  p(HTML("Les coordonnées qui sont chargées par défaut sont celle du <a href='http://www.walhain.be/Loisirs/office-du-tourisme/les-curiosites-touristiques/le-centre-geographique-de-la-belgique' target=_blank'>centre géographique de la Belgique</a>. ")),
	  p(HTML("Changez les en montionnant celles de votre propre colombier et vous bénéficierez d'un calcul adapté à votre localisation personnelle. ")),
	  p(HTML("Le format à utiliser est le format <a href='https://fr.wikipedia.org/wiki/WGS_84' target='_blank'>WGS84</a>, avec un <strong>point comme symbole décimal</strong>,et non une virgule.")),
	  p(HTML("<strong>Rappel :</strong> Vos coordonnées figurent sur votre carte de membre RFCB.")),
    ...
    HTML('<li><a href="http://www.euronamur.net/Pigeon/distances.html" target="_blank">http://www.euronamur.net/Pigeon/distances.html</a> : tableurs Excel de calcul des distances entre colombier et lieux de lâcher, et de vitesse des pigeons.</li>'),
	  HTML('<li><a href="http://www.movable-type.co.uk/scripts/latlong.html" target="_blank">http://www.movable-type.co.uk/scripts/latlong.html</a> : tableurs Excel et méthodes de calcul des distances entre deux coordonnées décimales.</li>'),
