...
# Define UI for zplot
shinyUI(navbarPage("Belgium Pigeon Racing Map",
  tabPanel(uiOutput('uiPanelTitle'),
  fluidPage(
  ...
  tags$head(
    ...
    tags$style(type='text/css', "select#selection,select#mapzones,select#racedist,select#language { width: 200px; display : inline; }"),
  ...
  fluidRow(
    column(3,
      wellPanel(
        selectInput("language",uiOutput('uiSBlanguage'),list("English" = "en","Français" = "fr", "Nederlands" ="nl"),selected = "fr",selectize=FALSE),#Must be defined in UI.R instead of server.R to avoid error of type "trying to select more than one value because input$language is only set after server.R load
        uiOutput('uiSBtop'),
...
tabPanel(uiOutput('uiCreditsTitle'),
         fluidPage(
           tags$head(
